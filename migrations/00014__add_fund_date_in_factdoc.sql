...
CREATE TABLE fact_documents (
    ...
    user_comment VARCHAR (255),
    foreign_uuid VARCHAR,
    funded_month INTEGER        AS (CASE WHEN payment_type = 'REFUND' THEN 1 WHEN funded_at IS NULL THEN strftime('%m', doc_date) ELSE strftime('%m', funded_at) END),
...
INSERT INTO fact_documents SELECT id, year_n, kgrbs, doc_number, doc_date, NULL, status_uid, payment_type, user_comment, foreign_uuid FROM fact_documents_backup;
